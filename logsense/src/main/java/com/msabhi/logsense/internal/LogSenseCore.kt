...
internal class LogSenseCore private constructor(
    ...
    private fun start() {
        ...
        val detector = AnalyticsDetector(config, { database.eventDao() }, scope)
        scope.launch { LogcatReader(buffer, detector::process).run() }
        ...
        scope.launch {
            ...
            if (config.showNotification) Notifications.postCapture(appContext)
        }
    }
